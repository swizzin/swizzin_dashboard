...
boottimeutc = datetime.datetime.utcfromtimestamp(boottimestamp).strftime(
    "%b %d, %Y %H:%M:%S"
)
        ...
        try:
            ...
            try:
                ...
                current_app.logger.debug(
                    "Encountered an error while loading the profile for %s: %s",
                    application,
                    e,
                )
                continue
...
def is_process_running(application, username, systemd=False, procs=False):
    ...
    if systemd is True:
        if application.endswith("@"):
            ...
            service = application
        returncode = sp.run(
            ("systemctl", "is-active", service), stdout=sp.DEVNULL
        ).returncode
        if returncode == 0:
            result = True
    else:
...
def is_application_enabled(application, user):
    if application.endswith("@"):
        ...
        result = os.path.exists(
            "/etc/systemd/system/multi-user.target.wants/{application}.service".format(
...
def systemctl(function, application):
    if function in ("enable", "disable"):
        result = sp.run(
            ("sudo", "systemctl", function, "--now", application), stdout=sp.DEVNULL
        ).returncode
    else:
        result = sp.run(
            ("sudo", "systemctl", function, application), stdout=sp.DEVNULL
        ).returncode
    return result
...
def network_quota_usage(username):
    quota = (
        sp.Popen(
            ("sudo", "/etc/swizzin.xl/add-on/panelquotas.sh", "json", username),
            stdout=sp.PIPE,
        )
        .communicate()[0]
        .decode("utf-8")

    ...
    with app.app_context():
        ...
        while True:
            ...
            if total_prev > 0:
                ...
                emit(
                    'speed',
                    {'interface': interface, 'tx': tx_speed, 'rx': rx_speed, 'total': total_speed},
                    namespace='/websocket',
                    broadcast=True,
                )
            time.sleep(1)
...
def authenticate():
    ...
    return flask.Response(
        ...
        401,
        {'WWW-Authenticate': 'basic realm="{0}"'.format(current_app.config['FLASK_AUTH_REALM'])},
    )
...
def vnstat(user):
    ...
    return flask.render_template(
        ...
        alltime=statsa,
        colnames=columns,
        top=top if app.config['NETWORK_INFO_TOP'] else None,
    )

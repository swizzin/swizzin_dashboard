...
socketio = SocketIO(
    app, path="{}/socket.io".format(app.config["URLBASE"]), async_mode=async_mode
)
...
def io_wait(app):
    ...
    with app.app_context():
        while True:
            ...
            readstats.close()
            iowait = "{0:.1f}".format(
                ((iowaitd - iowait) * 100 / tick) / numcpu / interval
            )
...
def unauthorized(e):
    if app.config["FORMS_LOGIN"]:
        ...
        if not urlbase == "" and not urlbase.endswith("/"):
            urlbase = urlbase + "/"
        if flask.request.referrer == "{host}{urlbase}login".format(
            host=flask.request.host_url, urlbase=urlbase
        ):
            return authenticate()
        elif flask.request.referrer == "{host}{urlbase}login/auth".format(
            host=flask.request.host_url, urlbase=urlbase
        ):
            return authenticate()
...
def index(user):
    ...
    return flask.render_template(
        "index.html",
        title="{user} - swizzin dashboard".format(user=user),
        user=user,
        pages=pages,
        quota=quota,
        vendor=vendor,
    ...
    with thread_lock:
        if thread is None:
            thread = socketio.start_background_task(
                current_speed, (flask.current_app._get_current_object())
            )
    with thread2_lock:
        if thread2 is None:
            thread2 = socketio.start_background_task(
                io_wait, (flask.current_app._get_current_object())
            )
...
def netdataproxy(user, p=""):
    ...
    if flask.request.method == "GET":
        if flask.request.args:
            ...
            resp = requests.get(
                "{SITE}?{querystring}".format(SITE=SITE, querystring=querystring)
            )
        else:
...
def loadavg(user):
    ...
    return flask.jsonify(
        {"1m": loadavg[0], "5m": loadavg[1], "15m": loadavg[2], "perutil": perutil}
    )
...
def vnstat(user):
    ...
    if vnstat_jsonversion == "1":
        ...
        qm = "month"
        qt = "top"
        thisday = vnstat_data(interface, "d")["interfaces"][0]["traffic"]["day"][-1][
            "id"
        ]
        thismonth = vnstat_data(interface, "m")["interfaces"][0]["traffic"]["month"][
            -1
        ]["id"]
...
def ram_stats(user):
    ...
    perutil = "{0:.2f}".format(
        (ramstats["MemTotal"] - ramstats["MemAvailable"]) / ramstats["MemTotal"] * 100
    )
    return flask.jsonify(
        {
            "ramtotal": ramtotal,
...
def network_quota(user):
    ...
    return flask.jsonify(
        {"nettotal": total, "netused": used, "netfree": free, "perutil": usage}
    )
